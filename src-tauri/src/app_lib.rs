            ...
            #[cfg(desktop)]
            {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    if matches!(
                        ...
                        crate::app_lib::types::RemoteBackendProvider::Tcp
                    ) {
                        if matches!(settings.backend_mode, crate::app_lib::types::BackendMode::Remote)
                        {
                            let state = app_handle.state::<state::AppState>();
                            ...
                            if let Ok(status) = tailscale::tailscale_daemon_status(state).await {
                                if matches!(status.state, crate::app_lib::types::TcpDaemonState::Running)
                                {
                                    let state = app_handle.state::<state::AppState>();
                                    let _ = tailscale::tailscale_daemon_start(state).await;
                                }
